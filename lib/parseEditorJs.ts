...
const parseEditorJson = (content: string): EditorJsResponse | null => {
	try {
		...
		invariant(data && 'blocks' in data);
		invariant(Array.isArray(data.blocks));
		invariant(data.blocks.every((item) => 'type' in item && 'data' in item));
		return data;
	} catch (err) {
		console.error(err);
		return null;
...
export const parseEditorJsToHtml = (content: string) => {
	...
	return Xss(html, { css: false, allowCommentTag: false });
};
